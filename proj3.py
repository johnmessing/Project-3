...
selected_crypto = st.selectbox('Select Cryptocurrency', ['bitcoin', 'ethereum', 'litecoin'])
st.write("")
st.write("")
...
if st.button('Forecast Prices'):
    ...
    if action == "Buy":
        st.markdown("**Buy** - The forecasted price is higher than the current price :money_with_wings:.")
        st.text("Hold")
    else:
        st.text("Buy")
        st.markdown("**Hold** - The forecasted price does not significantly exceed the current price :heavy_exclamation_mark:.")
